...
class sfValidatorBlacklist extends sfValidatorBase {
    ...
    protected function configure($options = array(), $messages = array()) {
        ...
        $this->addRequiredOption('reserved_values');
        $this->addOption('case_sensitive', true);
        $this->addMessage('forbidden', 'Value "%value%" is forbidden');
        $this->addMessage('reserved', 'Value "%value%" is reserved');
    }
    ...
    protected function doClean($value) {
        ...
        $reservedValues = $this->getOption('reserved_values');
        if ($reservedValues instanceof sfCallable) {
            $reservedValues = $reservedValues->call();
        }
        ...
        if (in_array($checkValue, $reservedValues)) {
            throw new sfValidatorError($this, 'reserved', array('value' => $value));
        }

        foreach ($forbiddenValues as $ch_val) {
                if (strpos($checkValue, $ch_val) !== false) {
