...
class eCrop
{
    ...
    private function createThumbImageResource()
    {
        ...
        $im = imagecreatetruecolor($this->getThumbWidth(), $this->getThumbHeight());
        $color = imagecolorallocate($im, 255, 255, 255);
        imagefilledrectangle($im, 0, 0, $this->getThumbWidth(), $this->getThumbHeight(), $color);

        $this->thumbImageResource = $im;
    ...
    public function crop($thumbDest, $free = true)
    {
        ...
        {
            $resize = $this->getThumbWidth() / $this->sourceWidth;
            //echo "resize:".$resize.' * ';
            $y_size = $this->sourceHeight * $resize;
            //echo "y_size:".$y_size.' * ';
            $div_height = ($this->getThumbHeight() - $y_size) / 2;
            //echo "div:".$div_height." * ";
        ...
        {
            $resize = $this->getThumbHeight() / $this->sourceHeight;
            //echo "resize:".$resize.' * ';
            $x_size = $this->sourceWidth * $resize;
            //echo "y_size:".$y_size.' * ';
            $div_height = ($this->getThumbWidth() - $x_size) / 2;
            //echo "div:".$div_height." * ";
